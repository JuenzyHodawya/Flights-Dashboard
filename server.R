server <- function(input, output, session) {
  ...
  showNotification("Created by Juenzy Hodawya", duration = 5, type = "error")
...
# Total Flights (server) ------------------------------------------
  output$total_flights <- renderValueBox({
    ...
    base_flights() %>%
      ...
      as.integer() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("plane-departure"), color = "purple",subtitle = "Number of Flights")
  })
...
# Avg per Day (server) --------------------------------------------
  output$per_day <- renderValueBox({
    ...
    base_flights() %>%
      ...
      round() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("balance-scale"), color = "yellow",subtitle = "Average Flights per day") 
  })
...
# Percent delayed (server) ----------------------------------------
  output$percent_delayed <- renderValueBox({
    base_flights() %>%
      ...
      round() %>%
      paste0("%") %>%
      valueBox(icon = icon("clock"), color = "maroon", subtitle = "Flights delayed")
  })
...
# Top airlines (server) -------------------------------------------
  output$top_airlines <- renderD3({
    # The following code runs inside the database
    base_flights() %>%
      group_by(carrier, airline) %>%
      tally() %>%
      collect() %>%
      arrange(desc(n)) %>%
      head(10) %>%
      arrange(airline) %>%
      mutate(airline = str_sub(airline, 1, 30)) %>%
      rename(
        x = carrier,
        y = n,
        label = airline
